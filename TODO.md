...
- [ ] [confidence: high] **Dependabot security updates が無効。** GitHub の security updates を有効化し、脆弱性修正 PR を自動生成させる
...
- [ ] [confidence: medium] `repoask-core` に `InvestigationOverview`, `PublicApiCard`, `PublicTypeCard`, `TestCard`, `HintCard`, `RenderBudget` を追加する
- [ ] [confidence: medium] parser 出力に `CommentInfo` / `ExportInfo` 相当の metadata を追加し、JSDoc / rustdoc / plain comment を構造化して保持する。overview/search/inspect で使うのは `symbol-local`, deterministic, compact, actionable な field に限定する
- [ ] [confidence: medium] parser が `CommentNormalizationStatus` を出し、language/source/style ごとの comment coverage を reference parser 比較で集計できるようにする
- [ ] [confidence: medium] `repoask-core` に overview 集約ロジックを追加し、public API / type / linked tests を deterministic に返せるようにする
- [ ] [confidence: medium] `repoask-repo` に `overview(spec, config)` / `overview_with_report(...)` を追加し、既存 cache/index build パスを `search` と共有する
- [ ] [confidence: high] CLI に `overview` を追加し、README 要約ではなく public API / type / public API tests を優先表示する
- [ ] [confidence: medium] WASM API に overview JSON を返す入口を追加する
- [ ] [confidence: medium] `index_store` の format version を bump し、overview 系 metadata の roundtrip test を追加する
- [ ] [confidence: medium] fixture repo で `overview` の snapshot / ranking test を追加する（public API priority, linked test priority, comment fallback）
...
- [ ] [confidence: medium] `RenderBudget` による section 別 truncation を実装し、情報過多/不足の両方を避ける
...
- [ ] [confidence: low] public API / export surface の判定を parser ごとに揃え、overview を implementation map と API map に分けるか再検討する
...
- [ ] [confidence: high] tree-sitter: Ruby の `module` が `SymbolKind::Class` として返される — 専用の `Module` kind を追加するか検討
- [ ] [confidence: medium] markdown: MDX (JSX in markdown) のコンポーネント記法を認識していない
- [ ] [confidence: high] markdown: 空セクション(見出し直後に次の見出し) がゼロ内容の `DocSection` として生成される
- [ ] [confidence: high] doc comment 抽出: Python の docstring (`"""..."""`) を tree-sitter で取得していない — AST の `expression_statement > string` ノードを処理する必要あり
- [ ] [confidence: medium] 各言語の comment fixture corpus がない — JSDoc / rustdoc / Python docstring / plain comment の representative case を揃え、reference parser 比較 coverage の基盤にする
...
- [ ] [confidence: high] CLI に `overview` が未実装 — README 要約ではなく、公開 API / 型 / 公開 API のテストを優先して返す
...
- [ ] [confidence: high] fixtures ベースの E2E テストがない — サンプルリポジトリで `parse_directory` → `InvertedIndex::build` → `search` を通しで検証する
- [ ] [confidence: high] tree-sitter の Java / C / C++ / Ruby のテストが未作成
...
- [x] ~~**similarity-rs を pre-commit に追加。**~~ → prek.toml に `similarity-fn` (threshold 0.96) + `similarity-types` (threshold 0.99) hook 追加済み。`--fail-on-duplicates` で常時ブロック
...
- [x] ~~**integration test: parse → index → search E2E。**~~ → line 195 の fixtures ベース E2E 項目に統合
