...
- [x] ~~`repoask-wasm`: wasm-bindgen エントリポイント~~ → `RepoIndex` (addFile/build/search/docCount) 実装済み。ただし現在 `wasm-pack build` は失敗する（`wasm/Cargo.toml` の `wasm-opt = ["-O", "--enable-bulk-memory-opt"]` が binaryen 125 で未対応）。`wasm/pkg/` には 2026-03-23 時点のビルド成果物が残っている
- [ ] [confidence: high] **WASM ビルドの修正。** `wasm/Cargo.toml` の `wasm-opt` 設定を `--enable-bulk-memory-opt` → `--enable-bulk-memory` に変更するか `wasm-opt = false` に切り替え、`wasm-pack build` が通る状態を復旧する
