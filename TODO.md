...
- [ ] **`oxc.rs` — `extract_from_var_decl` ↔ `extract_class_methods` (95%)。** AST パターンマッチ + ctx.push の構造的類似。ドメイン的に別処理なので無理な共通化は避ける
- [ ] **`#[cfg(test)]` 内ヘルパー (`make_symbol`/`make_doc`, 94%)。** `--skip-test` の制限。similarity-rs の改善を待つか、テストヘルパーを別ファイルに切り出して `--exclude` する
...
- [ ] **`missing_docs` を `warn` → `deny` に昇格。** 全 pub items にドキュメント追加済みのため昇格可能
...
- [x] ~~**cargo-machete を CI に追加。**~~ → CI Gate 1 (check) に `cargo machete` ステップ追加済み
...
- [ ] **direnv 連携。** `.envrc` に `use flake` を書いて `cd` 時に自動で devShell に入る（現状は手動で `nix develop` が必要）
...
- [x] ~~**cargo-machete を devShell に追加。**~~ → 初期から `flake.nix` に含まれていた
